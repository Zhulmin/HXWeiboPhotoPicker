...
@implementation MiOCollectionView
...
- (void)gestureRecognizerBegan:(UILongPressGestureRecognizer *)longPgr
{
    ...
    MiOPhotoSubViewCell *cell = (MiOPhotoSubViewCell *)[self cellForItemAtIndexPath:self.originalIndexPath];
...
- (void)handelItemInSpace {
    ...
    for (NSIndexPath *indexPath in totalArray) {
        MiOPhotoSubViewCell *sectionLastCell = (MiOPhotoSubViewCell *)[self cellForItemAtIndexPath:indexPath];
...
- (void)moveCell{
    ...
    for (MiOPhotoSubViewCell *cell in [self visibleCells]) {
