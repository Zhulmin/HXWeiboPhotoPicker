...
typedef enum : NSUInteger {
    MiOPhotoManagerSelectedTypePhoto = 0, // 只选择图片
    MiOPhotoManagerSelectedTypeVideo, // 只选择视频
    MiOPhotoManagerSelectedTypePhotoAndVideo // 图片和视频一起
} MiOPhotoManagerSelectedType;

@interface MiOPhotoManager : NSObject
...
/**
 是否把相机功能放在外面 默认 NO   使用 MiOPhotoView 时有用
 */
...
/**
 当选择类型为 MiOPhotoManagerSelectedTypePhotoAndVideo 时 此属性为YES时 选择的视频会跟图片分开排  反之  视频和图片混合在一起排
 */
...
//------//
@property (assign, nonatomic) MiOPhotoManagerSelectedType type;
...
@property (strong, nonatomic) MiOAlbumModel *tempAlbumMd;


/**
 MiOPhotoManagerSelectedTypePhoto            // 只选择图片 - 默认类型
 MiOPhotoManagerSelectedTypeVideo            // 只选择视频
 MiOPhotoManagerSelectedTypePhotoAndVideo    // 图片视频一起选
