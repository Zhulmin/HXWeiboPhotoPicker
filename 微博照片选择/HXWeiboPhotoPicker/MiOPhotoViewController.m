...
@property (strong, nonatomic) MiOAlbumModel *albumModel;
...
@implementation MiOPhotoViewController
...
- (void)getAlbumPhotos {
    MiOAlbumModel *model = self.albums.firstObject;
...
- (void)goCameraVC {
    if (self.manager.goCamera) {
        ...
        if (self.manager.showFullScreenCamera) {
            MiOFullScreenCameraViewController *vc = [[MiOFullScreenCameraViewController alloc] init];
            vc.delegate = self;
...
- (void)pushAlbumList:(UIButton *)button {
    ...
    if (button.selected) {
        if (self.manager.selectedList.count > 0) {
            for (MiOAlbumModel *albumMd in self.albums) {
                albumMd.selectedCount = 0;
            }
            for (MiOPhotoModel *photoModel in self.manager.selectedList) {
                for (MiOAlbumModel *albumMd in self.albums) {
            ...
            if (self.manager.selectedCameraList.count > 0) {
                MiOAlbumModel *albumMd = self.albums.firstObject;
...
//            self.rightBtn.layer.borderWidth = 0.5;
    ...
    MiOAlbumTitleButton *titleBtn = [MiOAlbumTitleButton buttonWithType:UIButtonTypeCustom];
    [titleBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [titleBtn setImage:[MiOPhohoTools hx_imageNamed:@"headlines_icon_arrow"] forState:UIControlStateNormal];
    ...
    [self.collectionView registerClass:[MiOPhotoViewCell class] forCellWithReuseIdentifier:PhotoViewCellId];
    ...
    if (!self.manager.singleSelected) {
        ...
        if (self.manager.type == MiOPhotoManagerSelectedTypeVideo) {
        ...
        if (self.manager.selectedPhotos.count > 0 && self.manager.isOriginal) {
            ...
            CGFloat totalW = [MiOPhohoTools getTextWidth:[NSString stringWithFormat:@"(%@)",self.manager.photosTotalBtyes] withHeight:originalBtnH fontSize:14];
    ...
    MiOAlbumListView *albumView = [[MiOAlbumListView alloc] initWithFrame:CGRectMake(0, -340, width, 340)];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath { 
    MiOPhotoViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:PhotoViewCellId forIndexPath:indexPath];
    if (!self.manager.singleSelected) {
        cell.iconDic = self.manager.photoViewCellIconDic;
    }
    MiOPhotoModel *model = self.objs[indexPath.item];
        ...
        if (self.responseTraitCollection) {
            if (self.responseForceTouchCapability) {
                if (self.isCapabilityAvailable) {
                    MiOPhotoViewCell *myCell = (MiOPhotoViewCell *)cell;
                    MiOPhotoModel *model = self.objs[indexPath.item];
...
- (UIViewController *)previewingContext:(id<UIViewControllerPreviewing>)previewingContext viewControllerForLocation:(CGPoint)location {
    MiOPhotoViewCell *cell = (MiOPhotoViewCell *)previewingContext.sourceView;
    NSIndexPath *index = [self.collectionView indexPathForCell:cell]; 
    if (!cell || cell.model.type == HXPhotoModelMediaTypeCamera) {
        return nil;
    }
    MiOPhotoModel *model = self.objs[index.item];
    ...
    __weak typeof(self) weakSelf = self;
    [MiOPhohoTools getPhotoForPHAsset:model.asset size:CGSizeMake(model.endImageSize.width * 1.5, model.endImageSize.height * 1.5) completion:^(UIImage *image, NSDictionary *info) {
    ...
    if (self.manager.singleSelected) {
        if (model.type == HXPhotoModelMediaTypeCameraVideo || model.type == HXPhotoModelMediaTypeVideo) {
            MiOVideoPreviewViewController *showVC = [[MiOVideoPreviewViewController alloc] init];
            showVC.preferredContentSize = model.endImageSize;
            showVC.isTouch = YES;
            [showVC.view addSubview:self.previewImg];
            return showVC;
        }else {
            MiOPhotoEditViewController *vc = [[MiOPhotoEditViewController alloc] init];
    ...
    if ((model.type == HXPhotoModelMediaTypePhoto || model.type == HXPhotoModelMediaTypePhotoGif) || (model.type == HXPhotoModelMediaTypeCameraPhoto || model.type == HXPhotoModelMediaTypeLivePhoto)) {
        MiOPhotoPreviewViewController *showVC = [[MiOPhotoPreviewViewController alloc] init];
        showVC.preferredContentSize = model.endImageSize;
        showVC.isTouch = YES;
        [showVC.view addSubview:self.previewImg];
        return showVC;
...
- (void)previewingContext:(id<UIViewControllerPreviewing>)previewingContext commitViewController:(UIViewController *)viewControllerToCommit {
    ...
    if (!self.manager.singleSelected) {
        if (model.type == HXPhotoModelMediaTypeVideo || model.type == HXPhotoModelMediaTypeCameraVideo) {
            MiOVideoPreviewViewController *vc = (MiOVideoPreviewViewController *)viewControllerToCommit;
            ...
            MiOPhotoPreviewViewController *vc = (MiOPhotoPreviewViewController *)viewControllerToCommit;
        ...
        if (model.type == HXPhotoModelMediaTypeVideo || model.type == HXPhotoModelMediaTypeCameraVideo) {
            ...
            MiOPhotoEditViewController *vc = (MiOPhotoEditViewController *)viewControllerToCommit;
            vc.delegate = self;
        }
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    MiOPhotoViewCell *cell = (MiOPhotoViewCell *)[self.collectionView cellForItemAtIndexPath:indexPath];
    ...
    if (model.type == HXPhotoModelMediaTypeCameraPhoto) {
        ...
        if (self.manager.selectedPhotos.count != self.manager.photoMaxNum) {
            if (!self.manager.selectTogether) {
                if (self.manager.selectedList.count > 0) {
                    MiOPhotoModel *phMd = self.manager.selectedList.firstObject;
...
- (void)cellChangeLivePhotoState:(MiOPhotoModel *)model {
    MiOPhotoModel *PHModel = [self.manager.selectedList objectAtIndex:[self.manager.selectedList indexOfObject:model]];
    PHModel.isCloseLivePhoto = model.isCloseLivePhoto;
    
    MiOPhotoModel *PHModel1 = [self.manager.selectedPhotos objectAtIndex:[self.manager.selectedPhotos indexOfObject:model]];
...
- (void)didPhotoBottomViewClick:(HXPhotoBottomType)type Button:(UIButton *)button {
    if (type == HXPhotoBottomTyPepreview) { // 预览
        ...
        if (self.manager.selectedPhotos.count > 0) {
            ...
            MiOVideoPreviewViewController *vc = [[MiOVideoPreviewViewController alloc] init];
            MiOPhotoModel *model = self.manager.selectedVideos.firstObject;
            MiOPhotoViewCell *cell = (MiOPhotoViewCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:[self.objs indexOfObject:model] inSection:0]];
            vc.coverImage = cell.imageView.image;
            vc.isPreview = self.isPreview;
            vc.manager = self.manager;
            vc.delegate = self;
...
- (void)changeOriginalState:(BOOL)selected IsChange:(BOOL)isChange
{
    if (selected) { // 选中时
        ...
        __weak typeof(self) weakSelf = self;
        // 获取一组图片的大小
        [MiOPhohoTools FetchPhotosBytes:self.manager.selectedPhotos completion:^(NSString *totalBytes) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                if (!weakSelf.manager.isOriginal) {
                    return;
                }
                weakSelf.manager.photosTotalBtyes = totalBytes;
                CGFloat totalW = [MiOPhohoTools getTextWidth:[NSString stringWithFormat:@"(%@)",totalBytes] withHeight:originalBtnH fontSize:14];
...
- (UIActivityIndicatorView *)indica
{
    if (!_indica) {
        ...
        CGFloat indicaX = self.bottomView.originalBtn.titleLabel.frame.origin.x + [MiOPhohoTools getTextWidth:[NSBundle hx_localizedStringForKey:@"原图"] withHeight:self.bottomView.originalBtn.frame.size.height / 2 fontSize:14] + 5;
