...
- (void)setupUI {
    ...
    if (self.photoManager.singleSelecteClip) {
        ...
        self.editView = [[MiOPhotoEditView alloc] initWithFrame:self.bgView.frame];
...
- (void)setupModel {
    ...
    if (self.photoManager.singleSelecteClip) {
        ...
        if (self.model.type == HXPhotoModelMediaTypeCameraPhoto) {
            ...
            self.requestID = [MiOPhohoTools getPhotoForPHAsset:self.model.asset size:PHImageManagerMaximumSize completion:^(UIImage *image, NSDictionary *info) {
        ...
        if (self.model.type == HXPhotoModelMediaTypePhotoGif) {
            __weak typeof(self) weakSelf = self;
            [MiOPhohoTools FetchPhotoDataForPHAsset:self.model.asset completion:^(NSData *imageData, NSDictionary *info) {
            ...
            if (self.model.type == HXPhotoModelMediaTypeCameraPhoto) {
                ...
                if (self.model.previewPhoto) {
                    ...
                    if (imgHeight > imgWidth / 9 * 17) {
                        requestID = [MiOPhohoTools getPhotoForPHAsset:self.model.asset size:CGSizeMake(width, height) completion:^(UIImage *image, NSDictionary *info) {
                            weakSelf.imageView.image = image;
                        }];
                    }else {
                        requestID = [MiOPhohoTools getPhotoForPHAsset:self.model.asset size:CGSizeMake(self.model.endImageSize.width, self.model.endImageSize.height) completion:^(UIImage *image, NSDictionary *info) {
                            weakSelf.imageView.image = image;
                        }];
                    }
